                    ...
                    if (item.subMenu) {
                        return (
                            <SubMenu
                                ...
                                title={
                                    <Link href={item.url}>
                                        <a onClick={this.handleMenuClick()}>{item.text}</a>
                                    </Link>
                                }>
                                {item.subMenu.map((subItem) => (
                                    <Menu.Item key={subItem.text}>
                                        <Link href={subItem.url}>
                                            <a onClick={this.handleMenuClick()}>{subItem.text}</a>
                                        </Link>
                                    </Menu.Item>
                                ))}
                            </SubMenu>
                            ...
                            <Menu.Item key={item.text}>
                                {item.type === 'dynamic' ? (
                                    <Link
                                        ...
                                        l<a onClick={() => { Router.push(`${item.url}/[pid]`) }}>{item.text}</a>
                                    </Link>
                                ) : (
                                    <Link href={item.url} as={item.alias}>
                                        <a onClick={() => { Router.push(`${item.url}/[pid]`) }}>{item.text}</a>
                                    </Link>
                                )}
                            </Menu.Item>
                        );
